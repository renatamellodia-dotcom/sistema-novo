...
function Baixa({refresh}) {
  ...
  const [obs,setObs]=useState('');
  const [ok,setOk]=useState(false);
  const [mesBaixa,setMesBaixa]=useState(()=>date.slice(0,7));
  const products=db.products.filter(p=>p.ativo);
  ...
  return (
    <div>
      ...
      <div className="card mb4" style={{marginTop:16}}>
        <div style={{fontSize:14,fontWeight:700,marginBottom:8}}>Filtrar baixas por mês</div>
        <input
          className="df-in"
          style={{width:'100%'}}
          type="month"
          value={mesBaixa}
          onChange={e=>setMesBaixa(e.target.value)}
        />
      </div>

     {(() => {
  const baixas = getLS('moves', [])
    .filter(m=>m.type==='baixa' && (m.date || '').startsWith(mesBaixa))
    .slice()
    .reverse();
...
function Relatorios({tick}) {
  ...
  function exportarPeriodo() {
    const fmt = window.prompt('Exportar período:\n1 - CSV (Excel)\n2 - JSON completo do período\n\nDigite 1 ou 2:');
    if(!fmt) return;
    const hoje = new Date().toISOString().slice(0,10);
    const vendasPeriodo = db.sales.filter(s=>s.date>=applied.from&&s.date<=applied.to);
    const entradasPeriodo = db.moves.filter(m=>m.date>=applied.from&&m.date<=applied.to);
    const fechamentosPeriodo = db.closings.filter(c=>c.date>=applied.from&&c.date<=applied.to);

    if(fmt === '1') {
      const header = 'Data,Produto,Responsavel,Qtd,Unit,Total,ProductId\n';
      const rows = vendasPeriodo.map(s=>`${s.date},"${s.produto}",${s.responsavel},${s.qtd},${s.unit},${s.total},${s.productId}`).join('\n');
      const blob = new Blob(['\uFEFF'+header+rows], {type:'text/csv;charset=utf-8'});
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href=url; a.download=`bar-complexo-periodo-${applied.from}-a-${applied.to}-${hoje}.csv`; a.click();
      ...
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href=url; a.download=`bar-complexo-periodo-${applied.from}-a-${applied.to}-${hoje}.json`; a.click();
      URL.revokeObjectURL(url);
    }
  }
    ...
    <div>
      <div className="pg-hdr">
        ...
        <div style={{display:'flex',gap:6}}>
          <button className="btn btn-op btn-sm" style={{fontSize:12}} onClick={exportarPeriodo}>📋 Exportar Período</button>
...
<button className="btn btn-op btn-sm" style={{fontSize:12}} onClick={()=>{
...
function Conferencia({tick}) {
  ...
  const xlsMes = {};
  Object.entries(xlsData || {}).forEach(([d,v])=>{
    if((d || '').startsWith(mes)) xlsMes[d] = v;
  });
  ...
  const dates = [...new Set([...salesMes.map(s=>s.date), ...closingsMes.map(c=>c.date), ...Object.keys(xlsMes)])].sort();
  ...
  function salvarResultado(result, nomeArquivo) {
  ...
  async function handleFile(e) {
    ...
    function processXLSX(XLSX, buf) {
      ...
      let colValor = header.findIndex(h=>h.includes('TOTAL')||h.includes('VALOR')||h.includes('VENDIDO')||h.includes('AMOUNT')||h.includes('BRUTO')||h.includes('LIQUIDO'));
      ...
      salvarResultado(result, file.name);
    }
    ...
    try {
      ...
      if(window.XLSX) processXLSX(window.XLSX, new Uint8Array(buf));
      else {
  ...
  function limpar() {
    const semMes = {};
    Object.entries(xlsData || {}).forEach(([d,v])=>{ if(!(d || '').startsWith(mes)) semMes[d] = v; });
    setXlsData(semMes);
    setFileName('');
    setErro('');
    saveStone(semMes);
  ...
  return (
    <div>
      ...
      <p style={{fontSize:14,color:'#6b7280',marginBottom:16}}>Faça upload do relatório da maquininha e confira mês por mês, sem misturar abril com maio.</p>

      <div className="card mb4">
        <div style={{fontSize:14,fontWeight:700,marginBottom:10}}>Mês da conferência</div>
        <input className="df-in" style={{width:'100%',marginBottom:12}} type="month" value={mes} onChange={e=>setMes(e.target.value)}/>
        ...
        {fileName && !loading && !erro && <div style={{fontSize:13,color:'#16a34a',marginBottom:6}}>✅ {fileName} — {Object.keys(xlsMes).length} dias neste mês <span style={{marginLeft:8,fontSize:12,color:'#6b7280'}}>💾 salvo automaticamente</span></div>}
      ...
      <div className="sec">
        ...
        <div className="tbl-wrap">
          <table>
            <thead><tr><th>Data</th><th style={{color:'#2563eb'}}>Declarado Fechamento</th><th>Maquininha (XLS)</th><th style={{color:'#16a34a'}}>Dinheiro Esperado</th><th>Status</th></tr></thead>
            <tbody>{dates.length?dates.map(d=>{
              ...
              return <tr key={d} style={diverge?{background:'#fefce8'}:(!c?{background:'#f9fafb'}:{})}>
                ...
                <td>{!xls ? <span style={{fontSize:13,color:'#9ca3af'}}>—</span> : diverge ? <span style={{background:'#ef4444',color:'#fff',padding:'5px 10px',borderRadius:8,fontSize:12,fontWeight:700,whiteSpace:'nowrap'}}>⚠ DIVERGE {R(diff)}</span> : <span style={{background:'#16a34a',color:'#fff',padding:'5px 10px',borderRadius:8,fontSize:12,fontWeight:700}}>✓ OK</span>}</td>
              </tr>;
